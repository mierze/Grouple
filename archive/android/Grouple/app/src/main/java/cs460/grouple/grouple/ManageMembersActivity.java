...
public class ManageMembersActivity extends BaseActivity {
	...
	private void setRoles()
	{
		...
		for (User u : members)
			new getRoleTask().execute("http://mierze.gear.host/grouple/android_connect/check_role_group.php", Integer.toString(members.indexOf(u)), Integer.toString(group.getID()));
							...
							int id)
					{
						...
						for (int i = 0 ; i < size ; i++)
						{
							...
							if (friendsRole.equals("-"))
							{
								...
								new UpdateGroupMembersTask().execute("http://mierze.gear.host/grouple/"
							...
							else
							{
								...
								System.out.println("http://mierze.gear.host/grouple/android_connect/update_group_member.php, url1: " + friendsEmail + ", 2: no, 3: " + friendsRole +", 4: " + Integer.toString(g_id));
								new UpdateGroupMembersTask().execute("http://mierze.gear.host/grouple/android_connect/update_group_member.php", friendsEmail, "no", friendsRole, Integer.toString(g_id));
							}
						}
