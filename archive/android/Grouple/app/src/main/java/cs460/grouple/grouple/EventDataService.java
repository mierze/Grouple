...
public class EventDataService extends Service
{
	...
	public void fetchContent(final String fetch, Context context)
	{
		...
		if (fetch.equals(FETCH_TYPE.INFO.toString()))
			new getInfoTask().execute("http://mierze.gear.host/grouple/android_connect/get_event_info.php");
			...
			new getParticipantsTask().execute("http://mierze.gear.host/grouple/android_connect/get_event_participants.php?eid="
					+ event.getID());
		else if (fetch.equals(FETCH_TYPE.ITEMS.toString()))
			new getItemsTask().execute("http://mierze.gear.host/grouple/android_connect/get_items_tobring.php");
	}
