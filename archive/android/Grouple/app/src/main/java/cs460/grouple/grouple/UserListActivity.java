...
public class UserListActivity extends BaseActivity
{
	...
	private void fetchRole()
	{
		if (CONTENT.equals(CONTENT_TYPE.GROUP_MEMBERS.toString()) && group.inUsers(user.getEmail()))
			new getRoleTask().execute("http://mierze.gear.host/grouple/android_connect/check_role_group.php",
					Integer.toString(group.getID()));
		else if (event.inUsers(user.getEmail()))
			new getRoleTask().execute("http://mierze.gear.host/grouple/android_connect/check_role_event.php",
	...
	public void acceptButton(String email)
	{
		...
		EMAILBUFFER = email;
		new performActionTask().execute("http://mierze.gear.host/grouple/android_connect/accept_friend_request.php", EMAILBUFFER);
	...
	public void declineButton(String email)
	{
		...
		EMAILBUFFER = email;
		new performActionTask().execute("http://mierze.gear.host/grouple/android_connect/decline_friend_request.php", EMAILBUFFER);
		...
		else if (CONTENT.equals(CONTENT_TYPE.FRIENDS_CURRENT.toString()))
		{
			...
			new AlertDialog.Builder(this).setMessage("Are you sure you want to remove that friend?")
					.setCancelable(true).setPositiveButton("Yes", new DialogInterface.OnClickListener()
					{
						...
						{
							new performActionTask().execute("http://mierze.gear.host/grouple/android_connect/delete_friend.php",
									friendEmail);
						}
