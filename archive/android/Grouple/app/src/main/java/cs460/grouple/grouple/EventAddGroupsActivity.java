...
public class EventAddGroupsActivity extends BaseActivity
{
	...
	public void addGroupsButton(View view)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			new EventAddGroupTask().execute("http://mierze.gear.host/grouple/" + "android_connect/add_eventmember.php", ID, email,
					ID);
		}
		try
		{
	...
	private class EventAddGroupTask extends AsyncTask<String, Void, String>
	{
		...
		protected String doInBackground(String... urls)
		{
			...
			return GLOBAL.readJSONFeed("http://mierze.gear.host/grouple/android_connect/get_group_members.php?gid=" + tmpid, null);
		}
			...
			try
			{
				...
				if (jsonObject.getString("success").toString().equals("1"))
				{
					...
					for (int i = 0; i < jsonArray.length(); i++)
					{
						...
						new EventAddMemberTask().execute("http://mierze.gear.host/grouple/"
