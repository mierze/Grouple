...
public class EventProfileActivity extends BaseActivity
{
	...
	private void fetchData()
	{
		...
		new getUnreadEntityMessagesTask().execute(
				"http://mierze.gear.host/grouple/android_connect/get_unread_entitymessages.php",
				Integer.toString(event.getID()));
	}
	...
	private void updateItemChecklist()
	{
		for (EventItem item : items)
		{
			...
			{
				new updateItemChecklistTask().execute("http://mierze.gear.host/grouple/android_connect/update_item_checklist.php",
	...
	public void joinButton(View v)
	{
		...
		new JoinPublicTask().execute("http://mierze.gear.host/grouple/android_connect/join_public_event.php", user.getEmail(),
