<?php
	...
	$professionalStmt->bind_param('s', $email);
	...
	$entertainmentStmt->bind_param('s', $email);
	...
	$fitnessStmt->bind_param('s', $email);
	$fitnessStmt->execute();
	...
	$publicEventStmt->bind_param('s', $email);
	...
	$professionalCreateStmt->bind_param('s', $email);
	...
	$natureCreateStmt->bind_param('s', $email);
	...
	$entertainmentCreateStmt->bind_param('s', $email);
	...
	$socialCreateStmt->bind_param('s', $email);
	...
	$fitnessCreateStmt->bind_param('s', $email);
	...
	$helpingStmt->bind_param('s', $email);
	$helpingStmt->execute();
	...
	if ($fetches["socialCount"] > 0)
	{
		...
		$socialCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$socialUpdateStmt->bind_param('sss', $email, $level, $name);
		...
		else
		{
			...
			$socialUpdateStmt->bind_param('sss', $email, $name, $level);
			if ($socialUpdateStmt->execute())
			{
				if ($mysqli->affected_rows > 0)
				{
	...
	if ($fetches["fitnessCount"] > 0)
	{
		...
		$fitnessCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$fitnessUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["professionalCount"] > 0)
	{
		...
		$professionalCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$professionalUpdateStmt->bind_param('sss',$level, $email, $name);
	...
	if ($fetches["entertainmentCount"] > 0)
	{
		...
		$entertainmentCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$entertainmentUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["natureCount"] > 0)
	{
		...
		$natureCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$natureUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["publicEventCount"] > 0)
	{
		...
		$publicEventCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$publicEventUpdateStmt->bind_param('sss', $email, $level, $name);
		...
		else
		{
			...
			$publicEventUpdateStmt->bind_param('sss', $email, $name, $level);
			if ($publicEventUpdateStmt->execute())
			{
				if ($mysqli->affected_rows > 0)
				{
	...
	if ($fetches["totalCount"] > 0)
	{
		...
		$totalCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$totalUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["socialCount"] > 0 && $fetches["entertainmentCount"] > 0 && $fetches["professionalCount"] > 0 && $fetches["fitnessCount"] > 0 && $fetches["natureCount"] > 0)
	{
		...
		$roundedCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$roundedUpdateStmt->bind_param('sss', $email, $level, $name);
		...
		else
		{
			...
			$roundedUpdateStmt->bind_param('sss', $email, $name, $level);
			if ($roundedUpdateStmt->execute())
			{
				if ($mysqli->affected_rows > 0)
				{
	...
	if ($fetches["socialCreateCount"] > 0)
	{
		...
		$socialCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$socialCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["professionalCreateCount"] > 0)
	{
		...
		$professionalCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$professionalCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["entertainmentCreateCount"] > 0)
	{
		...
		$entertainmentCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$entertainmentCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["fitnessCreateCount"] > 0)
	{
		...
		$fitnessCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$fitnessCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["natureCreateCount"] > 0)
	{
		...
		$natureCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$natureCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["totalCreateCount"] >= 5)
	{
		...
		$totalCreateCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$totalCreateUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	if ($fetches["numItems"] > 0)
	{
		...
		$helpingCheckStmt->bind_param('sss', $email, $name, $level);
		...
		if($row_cnt > 0)
		{
			...
			if ($level > $old_level)
			{
				...
				$helpingUpdateStmt->bind_param('sss', $level, $email, $name);
	...
	//$updateSocialStmt = $mysqli-prepare("insert into badges (email, name, level, date) values ?, ?, ?, 'CURRENT_TIMESTAMP'");
	//$updateNatureStmt->bind_param('sss', $email, $natureName, $natureLevel);
	//if ($updateNatureStmt->execute())
	//{
		//if ($mysqli->affected_rows > 0)
		//{
			$response["success"] = 1;
			$response["message"] = "Successfully updated nature badge!";
			$response["numSocial"] = $fetches["socialCount"];
			$response["numProfessional"] = $fetches["professionalCount"];
			$response["numEntertainment"] = $fetches["entertainmentCount"];
			$response["numFitness"] = $fetches["fitnessCount"];
			$response["numNature"] = $fetches["natureCount"];
			$response["numTotal"] = $fetches["totalCount"];
			$response["numSocialCreate"] = $fetches["socialCreateCount"];
			$response["numProfessionalCreate"] = $fetches["professionalCreateCount"];
			$response["numEntertainmentCreate"] = $fetches["entertainmentCreateCount"];
			$response["numFitnessCreate"] = $fetches["fitnessCreateCount"];
			$response["numNatureCreate"] = $fetches["natureCreateCount"];
			$response["numTotalCreate"] = $fetches["totalCreateCount"];
			$response["numItems"] = $fetches["numItems"];
			echo(json_encode($response));
		//}
	//}
	//$updateNatureStmt->close();
	$mysqli->close();
