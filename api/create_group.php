<?php
	...
	if(!isset($_POST['name']) || !isset($_POST['about']) || !isset($_POST['public']) || !isset($_POST['creator']))
	{
		$response["success"] = -99;
		$response["message"] = "Missing required POST parameters (name, about, creator, public)";
	...
	$public = $_POST['public'];
	$stmt = $mysqli->prepare("INSERT INTO groups (g_name, about, creator, public) 
	VALUES (?,?,?,?)");
	if($stmt === false)
	{
		$response["success"] = 0;
	...
	if($stmt->execute())
	{
		...
		if($row_cnt > 0)
		{
			$addStmt = $mysqli->prepare("INSERT into g_members (email, g_id, send_date, rec_date) values (?, ?, 'CURRENT_TIMESTAMP', 'CURRENT_TIMESTAMP')");
			if($addStmt === false)
			{
				$response["success"] = 0;
			...
			$addStmt->bind_param('sss', $creator, $id);
			$addStmt->execute();
			$row_cnt = $addStmt->num_rows;
			if ($row_cnt > 0)
			{
				$response["success"] = 1;
				$response["message"] = "Group successfully created!";
				$stmt->fetch();//grab first row
				$response["id"] = $id;
			}
		}
	}
