<?php
	...
	{
		$id = $_POST['email'];
		$stmt = $mysqli->prepare("SELECT image_xhdpi FROM users WHERE email = ?");
		$stmt->bind_param('s', $id);
	}
	else if(isset($_POST['gid']))
	{
		$id = $_POST['gid'];
		$stmt = $mysqli->prepare("SELECT image_xhdpi FROM groups WHERE g_id = ?");
		$stmt->bind_param('i', $id);
	}
	else if(isset($_POST['eid']))
	{
		$id = $_POST['eid'];
		$stmt = $mysqli->prepare("SELECT image_xhdpi FROM events WHERE e_id = ?");
		$stmt->bind_param('i', $id);
	}
	else
	{
