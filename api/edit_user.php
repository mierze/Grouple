<?php
	...
	{
		$response["success"] = -99;
		$response["message"] = "Missing required POST parameters (first, last, age, about, location, gender or email)";
	...
	if(!empty($_FILES["image"]))
	{
		...
		$stmt = $mysqli->prepare("UPDATE users SET first = ?, last = ?, age = ?, about = ?, location = ?, gender = ?, image='$data_or',
		image_mdpi = '$data_m', image_hdpi = '$data_h', image_xhdpi = '$data_l', image_xxhdpi= '$data_xl' WHERE email = ?");
		$stmt->bind_param('sssssss', $_POST['first'],$_POST['last'],$_POST['birthday'],$_POST['about'],$_POST['location'], $_POST['gender'], $_POST['email']);
	}
	//update without image (has not been set by user)
	else
	{
		$stmt = $mysqli->prepare("UPDATE users SET first = ?, last = ?, age = ?, about = ?, location = ?, gender = ? WHERE email = ?");
		$stmt->bind_param('sssssss', $_POST['first'],$_POST['last'],$_POST['birthday'],$_POST['about'],$_POST['location'], $_POST['gender'], $_POST['email']);
